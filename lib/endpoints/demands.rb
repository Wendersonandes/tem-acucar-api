      ...
      put "/:id/cancel" do |id|
        ...
        if current_user != demand.user
          Notification.create({
            ...
            text: "Seu pedido <b>#{demand.name}</b> foi considerado impróprio pelos nossos moderadores e foi cancelado. Se foi um mal entendido, por favor entre em contato atraves do e-mail temacucar@gmail.com.",
          })
        end
        encode serialize(demand, :current_user)
      end
